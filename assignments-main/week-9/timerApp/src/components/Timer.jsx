...
const Timer = () => {
  ...
  const [initialTime, setInitialTime] = useState(0); // Initial time set by the user
  const [isRunning, setIsRunning] = useState(false); // Timer status
  const [editState, setEditState] = useState({ field: null, value: '' }); // Field being edited
  ...
  const handleEditField = (field) => {
    if (editState.field === field) {
      ...
      const newTimeInSeconds = calculateTime(newTime.hours, newTime.minutes, newTime.seconds);
